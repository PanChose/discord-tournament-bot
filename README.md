...
- **Веб-панель** (открывается в браузере): выбрать сервер и канал, отправить туда сообщение от имени бота.
- **Редактор сообщений** — почти всё, что поддерживает Discord:
    - Форматирование текста через панель инструментов: **жирный**, *курсив*, подчёркнутый, ~~зачёркнутый~~, ||спойлер (скрытый текст)||, `код`, блок кода, цитата, заголовки `#`/`##`/`###`, списки.
    - **Embed-карточка**: заголовок (+ссылка), описание, цвет полоски, автор, картинка, миниатюра, футер, метка времени, произвольные поля (в том числе «в строку»).
    - **Кнопки**: до 25 штук (автоматически раскладываются по 5 в ряд), 4 цвета + отдельный тип «Ссылка» (открывает URL). Обычные кнопки при нажатии просто подтверждают клик — своей логики на конкретную кнопку в проекте нет, это можно добавить отдельно под конкретную задачу.
