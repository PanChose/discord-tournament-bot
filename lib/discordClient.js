...
const client = new Client({
    // GuildEmojisAndStickers isn't privileged (no dev-portal toggle needed) —
    // it's what lets guild.emojis.cache stay populated/fresh for the panel's
    // custom-emoji picker.
...
async function handleTournamentCommand(interaction) {
    ...
    if (sub === "edit") {
        ...
        modal.addComponents(
            ...
            new ActionRowBuilder().addComponents(
                new TextInputBuilder()
                    .setCustomId("external_url")
                    .setLabel("External tournament link (Challonge etc.)")
                    .setStyle(TextInputStyle.Short)
                    .setValue(tournament.external_url || "")
                    .setRequired(false)
            )
        );
        return interaction.showModal(modal);
...
async function handleRegisterCommand(interaction, isRegister) {
    ...
    await interaction.reply({ ...buildRegistrationReply(result, isRegister, tournament), ephemeral: true });
...
// Builds the ephemeral reply for a Join/Leave click. When the tournament has an
// external_url, a real Link-style button is attached — Discord can't fire a bot
// event *and* auto-navigate off a single click, so this is the closest thing to
// "automatic": one big obvious button instead of a URL buried in plain text.
function buildRegistrationReply(result, isRegister, tournament) {
    const linkButton = tournament.external_url
        ? new ActionRowBuilder().addComponents(
              new ButtonBuilder().setStyle(ButtonStyle.Link).setURL(tournament.external_url).setLabel("🔗 Continue to Tournament")
          )
        : null;

    if (result.ok) {
        if (!isRegister) return { content: "Registration cancelled." };
        return {
            content: tournament.external_url
                ? "You're in! ✅ Your Discord tag was recorded — tap below to finish signing up:"
                : "You're registered! ✅",
    ...
    switch (result.reason) {
        case "not_open":
            return { content: "Registration isn't open for this tournament." };
        case "already_registered":
            return {
                content: tournament.external_url ? "You're already in! ✅ Here's the link again:" : "You're already registered.",
                components: linkButton ? [linkButton] : [],
            };
        case "full":
            return { content: "Sorry, the team limit has been reached." };
        case "not_registered":
            return { content: "You weren't registered for this tournament." };
        default:
            return { content: "Tournament not found." };
    }
}
...
async function sendRegistrationDM(user, tournament) {
    ...
    let text = `✅ You're registered for **${tournament.name}**!\nStarts: ${when}\nWe'll remind you before it starts.`;
    if (tournament.external_url) text += `\n\n🔗 Finish signing up here: ${tournament.external_url}`;
...
async function handleModalSubmit(interaction) {
    ...
    if (interaction.customId.startsWith("tour_edit:")) {
        ...
        const updated = tournamentsLib.updateTournament(id, {
            ...
            maxParticipants,
            description: interaction.fields.getTextInputValue("description").trim() || null,
            externalUrl: interaction.fields.getTextInputValue("external_url").trim() || null,
        });
...
// =========================================================================
// Button interactions (Join / Leave on the announcement)
// =========================================================================

async function handleButton(interaction) {
    const [ns, action, id] = interaction.customId.split(":");
    if (ns !== "tour" || (action !== "register" && action !== "unregister")) return;
...
function buildTournamentMessagePayload(tournament) {
    ...
    // Banner, thumbnail, and author icon can each independently be a pasted
    // URL or an uploaded data: URL — this turns the latter into its own
    // attachment (Discord embeds accept attachment://<filename> anywhere a
    // normal image URL goes) without repeating that branch three times.
    const files = [];
    const resolveImage = (dataOrUrl, fallbackName) => {
        if (!dataOrUrl) return undefined;
        if (!dataOrUrl.startsWith("data:")) return dataOrUrl;
        const { buffer, filename } = dataUrlToAttachment(dataOrUrl, fallbackName);
        files.push(new AttachmentBuilder(buffer, { name: filename }));
        return `attachment://${filename}`;
    };

    const image = resolveImage(data.image, "banner");
    if (image) embed.setImage(image);

    const thumbnail = resolveImage(data.thumbnail, "thumbnail");
    if (thumbnail) embed.setThumbnail(thumbnail);

    if (data.author) {
        embed.setAuthor({ name: data.author.name, iconURL: resolveImage(data.author.iconURL, "author_icon") });
    }

    const isOpen = tournament.status === "published";
    const isFull = tournamentsLib.slotCountFor(tournament, activeCount) >= tournament.max_participants;
    ...
    return {
        content: tournamentsLib.publishPingContent(tournament),
        embeds: [embed],
        components: [new ActionRowBuilder().addComponents(joinBtn, leaveBtn)],
        files,
    };
}
...
async function publishTournament(tournament) {
    ...
    // Render as "published" even though the DB row is still "draft" at this point —
    // otherwise the footer says "Draft" and the Join button gets built disabled
    // (isOpen would read false), even though the message is about to go live.
    const payload = buildTournamentMessagePayload({ ...tournament, status: "published" });
    const message = await channel.send(payload);
    const published = tournamentsLib.setPublished(tournament.id, tournament.channel_id, message.id);

    const reaction = tournamentsLib.reactionResolvable(published);
    if (reaction) {
        message.react(reaction).catch((err) => console.error("[discord] Failed to add auto-react emoji:", err.message));
    }

    // Get the real team count in right away instead of waiting for the next
    // ~4-minute sync tick — fire-and-forget so a slow/failed Matcherino call
    // never delays the publish response.
    if (published.matcherino_bounty_id) {
        require("./matcherinoSync")
            .syncOne(published)
            .catch((err) => console.error("[matcherino] Initial sync failed:", err.message));
    }
...
// Re-syncs the announcement's auto-react emoji after an edit — removes the
// bot's own previous reaction (if any) before adding the new one, so changing
// your mind about the emoji doesn't leave old ones stacking up on the message.
async function applyReaction(tournament) {
    if (!tournament.channel_id || !tournament.message_id) return;
    try {
        const channel = await client.channels.fetch(tournament.channel_id);
        const message = await channel.messages.fetch(tournament.message_id);
        for (const existingReaction of message.reactions.cache.values()) {
            if (existingReaction.me) await existingReaction.users.remove(client.user.id).catch(() => {});
        }
        const resolvable = tournamentsLib.reactionResolvable(tournament);
        if (resolvable) await message.react(resolvable);
    } catch (err) {
        console.error("[discord] Failed to update auto-react emoji:", err.message);
    }
}
...
// Posts a short "+N teams" / "-N slots" update when a Matcherino sync tick
// finds the entrants count changed since the last check.
async function postMatcherinoDelta(tournament, delta) {
    if (!tournament.channel_id || !delta) return;
    try {
        const channel = await client.channels.fetch(tournament.channel_id);
        const count = tournament.matcherino_entrants;
        const max = tournament.max_participants;
        const prizeSuffix =
            tournament.matcherino_prize_pool !== null && tournament.matcherino_prize_pool !== undefined
                ? ` Prize pool: ${tournamentsLib.formatPrizePool(tournament.matcherino_prize_pool)}.`
                : "";
        const text =
            delta > 0
                ? `📈 **+${delta}** team${delta === 1 ? "" : "s"} registered on Matcherino for **${tournament.name}** — ${count}/${max} slots taken.${prizeSuffix}`
                : `📉 **${delta}** slot${delta === -1 ? "" : "s"} opened up on Matcherino for **${tournament.name}** — ${count}/${max} slots taken.${prizeSuffix}`;
        await channel.send(text);
    } catch (err) {
        console.error("[discord] Failed to post Matcherino update:", err.message);
    }
}
...
async function sendReminderDM(userId, tournament) {
    const user = await client.users.fetch(userId);
    // Discord renders <t:...:R> as a live "in 2 hours"-style relative time —
    // no need to compute or pass a duration ourselves.
    const when = tournament.starts_at
        ? `<t:${Math.floor(tournament.starts_at / 1000)}:F> (<t:${Math.floor(tournament.starts_at / 1000)}:R>)`
        : "soon";
    await user.send(`⏰ Reminder: **${tournament.name}** starts ${when}!`);
}
...
// Custom server emojis, for the description editor's emoji picker — Discord
// requires `<:name:id>` (or `<a:name:id>` for animated ones) to render them
// outside of that same server, so we hand the panel exactly that syntax.
