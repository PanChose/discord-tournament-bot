...
async function handleRegisterCommand(interaction, isRegister) {
    ...
    await interaction.reply({ content: registrationResultMessage(result, isRegister, tournament), ephemeral: true });
...
function registrationResultMessage(result, isRegister, tournament) {
    if (result.ok) {
        if (!isRegister) return "Registration cancelled.";
        return tournament.external_url
            ? `You're in! ✅ Your Discord tag was recorded — now finish signing up here: ${tournament.external_url}`
            : "You're registered! ✅";
    }
...
// =========================================================================
// Button interactions (the single Join button on the announcement — cancelling
// a registration is done via /unregister, there's no button for it)
// =========================================================================

async function handleButton(interaction) {
    const [ns, action, id] = interaction.customId.split(":");
    if (ns !== "tour" || action !== "register") return;
    ...
    const result = tournamentsLib.registerParticipant(id, interaction.user.id, interaction.user.tag);
    await interaction.reply({ content: registrationResultMessage(result, true, tournament), ephemeral: true });

    if (result.ok) {
        sendRegistrationDM(interaction.user, tournament).catch(() => {});
        await refreshAnnouncementMessage(tournamentsLib.getTournament(id));
    }
}
