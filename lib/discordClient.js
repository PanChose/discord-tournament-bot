...
async function postMatcherinoDelta(tournament, delta) {
    ...
    try {
        ...
        const text =
            delta > 0
                ? `📈 **+${delta}** team${delta === 1 ? "" : "s"} registered on Matcherino for **${tournament.name}** — ${count}/${max} slots taken.`
                : `📉 **${delta}** slot${delta === -1 ? "" : "s"} opened up on Matcherino for **${tournament.name}** — ${count}/${max} slots taken.`;
