...
const { tournamentCommands, handleInteraction } = require("./commands");
const matcherinoSync = require("./matcherinoSync");
...
function attachHandlers() {
    ...
    // --- /verify, /tournament, /team, /leaderboard, and their buttons/modals/selects ---
    client.on("interactionCreate", async (interaction) => {
        if (interaction.isChatInputCommand()) {
            if (["verify", "tournament", "team", "leaderboard"].includes(interaction.commandName)) {
        ...
        if (interaction.isButton()) {
            ...
            // Not one of ours (customId doesn't start with tour:/verify:/team:) — falls
            // through to the panel's generic "acknowledge any button" behaviour below.
            try {
...
// Re-exported so server.js has one place to import bot-management helpers from.
module.exports.tournamentSystem = {
    matcherinoSync: require("./matcherinoSync"),
    teams: require("./teams"),
