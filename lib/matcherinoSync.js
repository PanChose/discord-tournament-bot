...
// Fetches the current entrants count and bracket status for one tournament,
// stores them, and silently refreshes the live announcement's fields — no
// per-change channel message, so registrations trickling in don't spam the
// channel. Exported so the web panel can also trigger it on demand (see
// /api/tournaments/:id/matcherino-sync).
async function syncOne(tournament) {
    if (!tournament.matcherino_bounty_id) return tournament;
    const discordClient = require("./discordClient");

    const { count, status } = await matcherino.fetchBracketInfo(tournament.matcherino_bounty_id);
    ...
    const updated = tournamentsLib.updateMatcherinoStats(tournament.id, { entrants: count, prizePool, status });

    if (updated.status === "published" || updated.status === "closed") {
