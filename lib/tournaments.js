...
function createTournament(guildId, organizerId, data) {
    ...
    db.prepare(
        `INSERT INTO tournaments
         (id, guild_id, channel_id, name, game, format, starts_at, max_participants, description, banner, external_url, color, status, reminder_hours, ping_role_id, organizer_id, matcherino_bounty_id, created_at, updated_at)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'draft', ?, ?, ?, ?, ?, ?)`
    ).run(
        id,
        guildId,
...
function updateMatcherinoCount(id, count) {
    db.prepare(`UPDATE tournaments SET matcherino_entrants = ?, matcherino_checked_at = ? WHERE id = ?`).run(count, Date.now(), id);
    return getTournament(id);
}
