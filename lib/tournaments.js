...
// Labels for the web panel's dashboard, which — unlike the embed's own
// footer — also reflects a linked Matcherino bracket's real lifecycle once
// it moves past registration.
...
// Buckets a Matcherino bracket status string ("scheduled", "in-progress",
// "completed", ...) into one of our two extra dashboard states, or null if
// it's still some pre-start state we don't need to distinguish from our own.
...
// The dashboard-facing status key: our own lifecycle status, upgraded to
// "in_progress"/"finalized" once a linked Matcherino bracket reports one of
// those — but never for a draft, which hasn't even been announced yet.
function dashboardStatusKey(tournament) {
    if (tournament.status === "draft") return "draft";
    return matcherinoStatusCategory(tournament.matcherino_status) || tournament.status;
}
...
function createTournament(guildId, organizerId, data) {
    ...
    db.prepare(
        `INSERT INTO tournaments
         (id, guild_id, channel_id, name, game, format, starts_at, max_participants, description, banner, external_url, color, status, reminder_at, ping_role_id, ping_on_publish, author_name, author_icon, thumbnail, organizer_id, matcherino_bounty_id, reaction_emoji_id, reaction_emoji_name, created_at, updated_at)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'draft', ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`
    ).run(
        id,
        guildId,
        ...
        data.pingOnPublish ? 1 : 0,
        data.authorName || null,
        data.authorIcon || null,
        data.thumbnail || null,
        organizerId,
        extractBountyId(data.externalUrl),
        data.reactionEmojiId || null,
        data.reactionEmojiName || null,
        now,
        now
    );
...
// Duplicates a tournament's branding/content into a brand-new draft — not a
// full copy: the start time, reminder time, and external/Matcherino link are
// all tied to the specific old event, so a clone starts without them rather
// than silently pointing at the same bracket or a stale date.
function cloneTournament(source, organizerId) {
    return createTournament(source.guild_id, organizerId, {
        ...
        color: source.color,
        pingRoleId: source.ping_role_id,
        pingOnPublish: Boolean(source.ping_on_publish),
        authorName: source.author_name,
        authorIcon: source.author_icon,
        thumbnail: source.thumbnail,
        reactionEmojiId: source.reaction_emoji_id,
        reactionEmojiName: source.reaction_emoji_name,
    });
}
...
const EDITABLE_FIELDS = {
    ...
    thumbnail: "thumbnail",
    reactionEmojiId: "reaction_emoji_id",
    reactionEmojiName: "reaction_emoji_name",
};
...
function updateTournament(id, data) {
    ...
    // Stored as INTEGER NOT NULL — coerce explicitly since better-sqlite3
    // refuses to bind a raw JS boolean.
    if (Object.prototype.hasOwnProperty.call(data, "pingOnPublish")) {
        sets.push("ping_on_publish = ?");
        values.push(data.pingOnPublish ? 1 : 0);
    }
...
function updateMatcherinoStats(id, { entrants, prizePool, status }) {
    if (prizePool === null || prizePool === undefined) {
        db.prepare(`UPDATE tournaments SET matcherino_entrants = ?, matcherino_status = ?, matcherino_checked_at = ? WHERE id = ?`).run(
            entrants,
            status ?? null,
            Date.now(),
            id
        ...
        db.prepare(
            `UPDATE tournaments SET matcherino_entrants = ?, matcherino_prize_pool = ?, matcherino_status = ?, matcherino_checked_at = ? WHERE id = ?`
        ).run(entrants, prizePool, status ?? null, Date.now(), id);
    }
    return getTournament(id);
}

// Published *or* closed tournaments with a Matcherino link — polled by
// lib/matcherinoSync.js. Closed (registration-wise) doesn't mean the bracket
// itself is done, so this keeps tracking matcherino_status past that point;
// only a draft (never announced, no message to refresh) is excluded.
function listPublishedWithMatcherino() {
    return db.prepare(`SELECT * FROM tournaments WHERE status IN ('published', 'closed') AND matcherino_bounty_id IS NOT NULL`).all();
}
...
// Builds the string discord.js's Message#react() expects: "name:id" for a
// custom server emoji, or the bare unicode character for a standard one.
// Returns null when no auto-react emoji is configured.
function reactionResolvable(tournament) {
    if (!tournament.reaction_emoji_name) return null;
    return tournament.reaction_emoji_id ? `${tournament.reaction_emoji_name}:${tournament.reaction_emoji_id}` : tournament.reaction_emoji_name;
}
...
function buildEmbedData(tournament, activeCount) {
    ...
    return {
        ...
        thumbnail: tournament.thumbnail || undefined,
        author: tournament.author_name ? { name: tournament.author_name, iconURL: tournament.author_icon || undefined } : undefined,
        fields,
        footer: STATUS_LABELS[tournament.status] || tournament.status,
    };
}

// The real role ping (if configured) that goes in the message's plain
// `content` alongside the embed — embeds never trigger a notification even
// with a mention inside them, so this is the only way to actually ping.
function publishPingContent(tournament) {
    return tournament.ping_on_publish && tournament.ping_role_id ? `<@&${tournament.ping_role_id}>` : undefined;
}
